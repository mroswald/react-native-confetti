...
class Confetti extends Component {
  ...
  constructor(props) {
    ...
    this.left = this.randomValue(0, this.windowWidth);
    let rotationOutput = this.randomValue(-220, 220) + 'deg';
    this._rotateAnimation = this._yAnimation.interpolate({
      inputRange: [0, this.windowHeight / 2, this.windowHeight],
      outputRange: ['0deg', rotationOutput, rotationOutput]
    });

    let xDistance = this.randomIntValue((this.windowWidth / 3) * -1, this.windowWidth / 3);
  ...
  componentDidMount() {
    ...
    Animated.timing(this._yAnimation, {
      ...
      toValue: this.windowHeight + 1.25,
